...
Usage: playlist_mp3_download <youtube_playlist_url>
...
OSS: A causa della codifica "strana" di download ho dovuto riprocessare
     ...
     un nome temporaneo, poi lo codifico con ffmpeg e cancello il file originale.

NOTA: da terminare migliorando la visualizzazione che a causa di ffmpeg è un casino.
      Sarebbe carino fare una sorta di interfaccia grafica carina. 
"""
...
if choice == "s":
    for i, video in enumerate(p.videos):
        ...
        os.system(f'ffmpeg -i "tmp_{video_name}.mp3" -c:a mp3 -hide_banner "{video_name}.mp3"')
